...
#FOR 2
        for i in range(self.iterations):
            if priorityQueue.isEmpty():                       #if the priority queue is not empty, continue
                return
            state = priorityQueue.pop()                       #Pop a state from the queue

            if not self.mdp.isTerminal(state):                #calculate the best Q-value if not a terminal state
                QValues = util.Counter()
                for action in self.mdp.getPossibleActions(state):
                    QValues[action] = self.computeQValueFromValues(state, action)
            ...
            for p in predecessors[state]:
                ...
                for action in self.mdp.getPossibleActions(p):    #find the best action with the highest QValue
                    QValues_p[action] = self.computeQValueFromValues(p, action)

                diff = abs(tempValues[p] - QValues_p[QValues_p.argMax()])   #diff = current value - best possible value
                if (diff > self.theta):
